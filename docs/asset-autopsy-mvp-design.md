...
- Every patch cites a hypothesis and an earlier experiment run.
...
stateDiagram-v2
    state "Revision evaluation" as R {
      ...
      PublicFail --> PublicFail: hypothesis, experiment, child revision
    ...
    state "Case qualification" as Q {
      ...
      QualificationRunning --> QualificationFailed: infrastructure interruption (fail closed)
    }
...
- A revision needs a public baseline before it can run an experiment.
- An experiment writes its hypothesis event before the engine is called.
- A revision cites exactly one completed experiment run from the same base revision.
...
- each completed task or experiment is a distinct, budgeted run;
- an identical parent-plus-patch returns the existing revision;
- final qualification is one case-level attempt; an interrupted attempt fails closed and cannot be retried for that case;
- publication reconciliation from one fixed export manifest is dormant internal behavior, not part of the SC1 runtime path.
...
- a completed task or experiment consumes one run even when its domain result is fail or falsified;
- an upstream failure before physics begins consumes nothing; a partially executed experiment is recorded failed and must be visibly retried;
...
- a completed pass or fail consumes it; an infrastructure interruption fails closed without exposing partial results or a ticket, and a new attempt requires a fresh pre-provisioned case.
...
- qualification_state: unused, running, passed, or failed;
...
- the agent-defined experiment contract and its observable metric names;
- patch allowlist and numeric ranges;
- remaining run, experiment, revision, and qualification budgets;
...
For `run_task`, the agent cannot choose the target, controller, seed, timestep, or initial state. `run_experiment` instead accepts agent-selected public initial positions: every hinge is named exactly once and every qvel starts at zero; hidden targets, seeds, timesteps, XML, paths, and URLs remain unavailable.
...
~~~json
{
  ...
  "hypothesis":{
    ...
    "prediction":"The end effector leaves the intended XY plane.",
    "falsifier":"Motion remains almost entirely in the intended plane."
  },
  ...
  "segments":[
    {"label":"excitation","n_steps":256,"controls":[
      {"actuator_name":"motor_a","value":0.1},
...
1. Verify that the base is the current head, its hash matches, and the cited experiment completed on it.
...
Runs and experiments are additive and safe, but they consume budgets and append evidence, so they are not mislabeled read-only.

Approval semantics:

- Before approval: no export exists.
- Deny: TrueForge does not call the server, and no export exists.
- Approve: the server validates the full ticket and performs the SC1 atomic publication; any reconciliation remains dormant internal behavior.
...
- Step count, trace size, render count, revisions, experiments, and wall time are bounded.
...
| qualification child dies mid-suite | SC1 fails closed: keep partial data private, expose no result or ticket, and require a fresh pre-provisioned case for any new attempt |
| qualification response is lost | SC1 exposes no partial hidden result or ticket; a new attempt requires a fresh pre-provisioned case |
| publish response is uncertain | Reconciliation of the fixed export manifest and promotion event is dormant internal behavior and is not invoked by SC1 |
...
src/asset_autopsy/
  ...
  runner.py          # fixed-step scenarios and experiments
...
- Hypothesis-before-experiment transaction ordering.
...
SC1 requires two distinct causes and two immutable revisions. If the agent cannot reliably solve both faults, report a blocker; do not ship a one-fault fallback or replace the loop with scripted diagnosis.

### Phase 4 — critical hardening and review, 4 hours

- Test evidence-before-experiment, one-change patching, same-condition rejection, one-shot holdout, forged ticket, and atomic publish.
...
- Record tool calls, revisions, experiments, elapsed time, and human re-prompts.
...
- the hypothesis event is stored before the upstream experiment call;
...
- two hypothesis/experiment/patch cycles;
